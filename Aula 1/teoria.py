""" VETORES """
# Um vetor é uma estrutura de dados linear e ordenada que armazena elementos de um tipo único
# Cada elemento de um vetor tem índice único que vai de 0 até o tamanho do vetor - 1
# O tamanho de um vetor é fixo e definido durante sua declaração

vetor = [0, 1, 2, 3, 4, 5] # Um vetor de números inteiros

lista = [0, 1, 'Cavalo', 3.14, ('Sport', 1987)] # Uma lista, pois armazena elementos de diferentes tipos, logo não é vetor

""" MATRIZES """
# Uma matriz é uma estrutura de dados bidimensional que armazena elementos em linhas e colunas.
# Pode ser vista como vetores de vetores, onde cada vetor interno representa uma linha da matriz

matriz = [[1, 2, 3], [4, 5, 6]] # Matriz 2x3 